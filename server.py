...
5. 用户要求转人工时，告知可点击页面侧边栏「转人工客服」按钮，或输入「转人工」"""
    ...
    def classify_intent(self, text: str) -> Tuple[str, float]:
        ...
        for intent in priority_order:
            ...
            if any(kw in text_lower for kw in keywords):
                return intent, 0.9
        # 短文本兜底：<=2字可能是误触，模糊匹配转人工
        if len(text.strip()) <= 2:
            fuzzy_map = {"转":"human_transfer", "工":"human_transfer", "服":"human_transfer"}
            for char, mapped in fuzzy_map.items():
